...
import {
  ...
  PopoverContent,
  PopoverTrigger,
} from "../../../components/popover"; // ShadCN Popover components
  ...
  return (
    <Popover>
      <PopoverTrigger>
        <Button
          variant="outline"
          className="flex items-center justify-between border-gray-300 p-2 hover:border-primary-600"
        >
          <div className="flex items-center space-x-2">
            <img src={config?.iconUrls?.sm ?? ""} className="w-5" />
            <span className="text-md">{truncatedAddress}</span>
            <span
              className={`badge ${config?.chainType === "mainnet" ? "bg-green-500" : "bg-purple-500"} text-white rounded-full px-2 py-1 text-xs`}
            >
              {config?.chainType}
            </span>
          </div>
      ...
      <PopoverContent className="p-4 bg-white shadow-lg rounded-lg w-80">
        ...
        <div className="mb-3 flex flex-col space-y-2">
          <Button asChild>
            <a
              href={config?.blockExplorerAddressPages[0]?.replace(
                "${address}",
                account?.address ?? ""
              )}
              target="_blank"
