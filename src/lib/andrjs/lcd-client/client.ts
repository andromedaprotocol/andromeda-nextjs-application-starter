...
export class LcdClient {
    ...
    async get<T>(path: string, options: ClientOptions = this.options): Promise<T> {
    ...
    async queryContractSmart<T, Q = unknown>(contractAddress: string, query: Q, options: ClientOptions = this.options): Promise<T> {
        const encodedMsg = Buffer.from(JSON.stringify(query)).toString("base64");
        return this.get<T>(`/cosmwasm/wasm/v1/contract/${contractAddress}/smart/${encodedMsg}`, options);
    }
}
